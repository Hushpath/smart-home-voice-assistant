...
```text
前端上传音频 -> 后端 ASR Provider -> transcript -> ASRPostCorrector -> DialectNormalizer -> MultiCommandParser -> CommandParser -> device_capabilities -> CommandExecutor -> command_logs
```
...
`ASRPostCorrector` 使用固定高置信错词表和当前用户的动态领域词表做保守后纠错。动态领域词表来自房间、设备、用户设备别名、场景、设备参数能力表和枚举值；例如可把 `冰香` 修正为 `冰箱`、`空气进化器` 修正为 `空气净化器`、`打开书房空气净化` 补全为 `打开书房空气净化器`。包含英文或数字的频道值不做模糊纠错，避免把 `CCTV5` 误改为其他频道。纠错结果通过 `asr_post_correction` 写入解析详情和日志 context。
...
`DialectNormalizer` 内仍保留少量识别文本常见错词纠正：`客厅等 -> 客厅灯`、`卧室等 -> 卧室灯`、`空条/空跳 -> 空调`、`电视及 -> 电视机`、`音凉 -> 音量`、`两度 -> 亮度`、`二十留 -> 二十六`。中文数字转换复用现有指令解析中的中文数字逻辑，例如 `三十 -> 30`、`八十 -> 80`、`二十六 -> 26`、`晚上八点 -> 20:00`。
...
- `asr`：输入来源、ASR provider、transcript、corrected_transcript、ASR 后纠错详情、音频时长、ASR 耗时和 raw ASR 结果。
...
```text
原始文本 -> ASR 后纠错 -> 文本标准化 -> 同义词归一 -> 中文数字转换 -> 意图打分 -> 房间/设备/参数匹配 -> 设备能力表校验 -> 置信度计算 -> 结构化结果
```
...
- 设备别名归一：电灯/灯光归一为灯，电视机归一为电视，冷气归一为空调，声音归一为音量；台灯、床头灯、扫地机器人、音箱、加湿器、空气净化器、智能插座、冰箱、烟雾传感器等设备按独立类型匹配。
...
- `set_brightness`：亮度、灯/台灯/床头灯、数字加分。
- `set_volume`：音量/声音、电视/音箱、数字加分。
- `set_property`：设备能力表中的参数别名、数值或枚举值加分。
...
- 房间和设备优先精确匹配。
- 精确匹配失败时使用别名匹配，例如冷气匹配为空调、电视机匹配为电视；包含关系设备名会优先保留长词，避免把“台灯”“床头灯”拆成普通“灯”。
- 别名匹配失败时使用 `difflib.SequenceMatcher` 做轻量模糊匹配，例如“打开客厅等”尽量匹配为客厅灯。
- 模糊匹配置信度过低时不会执行设备控制，会返回低置信度提示。

参数抽取与范围校验：

- 设备参数能力集中在 `app/services/device_capabilities.py`，支持数值型和枚举型属性。
- 空调温度仍按 `16-30` 校验；冰箱冷藏温度按 `2-8` 校验；亮度、音量、窗帘开合比例按 `0-100` 校验。
- 枚举型参数按设备能力表校验，例如排风扇风速 `低速/中速/高速`、冰箱模式 `保鲜/节能/速冷`、空调风量 `低速/中速/高速/自动`。
...
- 1 个默认家庭：默认家庭。
- 4 个房间：客厅、卧室、厨房、书房。
- 20 个设备：客厅灯、客厅空调、客厅电视、客厅窗帘、客厅扫地机器人、客厅音箱、卧室灯、卧室空调、卧室窗帘、卧室床头灯、卧室加湿器、厨房灯、厨房排风扇、厨房冰箱、厨房烟雾传感器、书房灯、书房空调、书房台灯、书房空气净化器、书房智能插座。
...
- 扩充口语语义归一和相对调节规则，例如“拉一半”“开大点”“冷一点”。
- 增加更多 ASR 后纠错测试语料和歧义保护规则。
- 增加前端页面联调后的接口契约测试。
- 增加操作日志筛选、分页和导出能力。
- 增加更完整的用户家庭/成员管理。
